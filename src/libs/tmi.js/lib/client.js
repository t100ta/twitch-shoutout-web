  ...
  // Messages with "tmi.twitch.tv" as a prefix..
    ...
    switch (message.command) {
      ...
      case "376": {
        ...
        const joinChannels = [
          ...new Set([...this.opts.channels, ...this.channels]),
        ];
        this.channels = [];
      ...
      case "USERNOTICE": {
        ...
        switch (msgid) {
          ...
          case "giftpaidupgrade": {
            const sender =
              tags["msg-param-sender-name"] || tags["msg-param-sender-login"];
          ...
          case "raid": {
            ...
            // Add loginName to support Japanese user name by t100ta
            const loginName = tags["msg-param-login"];
            // this.emit("raided", channel, username, viewers, tags);
            this.emit("raided", channel, username, viewers, loginName, tags);
            break;
          }
          // Handle ritual
          case "ritual": {
      ...
      case "RECONNECT":
        ...
        this.log.info(
          `Disconnecting and reconnecting in ${Math.round(this.reconnectTimer / 1000)} seconds..`
        );
...
client.prototype._onClose = function _onClose() {
  ...
  // Got disconnected from server..
    ...
    if (
      ...
      this.reconnections = this.reconnections + 1;
      this.log.error(
        `Could not connect to server. Reconnecting in ${Math.round(this.reconnectTimer / 1000)} seconds..`
      );
      this.emit("reconnect");
      setTimeout(() => {
        this.reconnecting = false;
